...
fn main() {
    ...
    let fileset = Arc::new(Mutex::new(crdt_fileset::FileSet::new(updater, site_id, path.as_path()).unwrap()));
    trace!("Setting up comms");
    let mut comms = communication::Client::new_standard(&url, &data, fileset.clone()).unwrap();
